  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const withLast = await Promise.all((txs ?? []).map(async (t: any) => {
        ...
        return { ...t, lastMessage: last, isSeller: t.seller_id === user.id }
      }))

      setRows(withLast.sort((a, b) => {
      ...
      <div style={{ padding: 16 }}>
        ...
        {rows.map(t => (
          <button key={t.id} onClick={() => router.push(`/mission/${t.id}/chat`)}
            style={{ display: 'block', width: '100%', textAlign: 'left', background: '#fff', border: '1px solid #E7EDEB', borderRadius: 14, padding: 14, marginBottom: 10, cursor: 'pointer' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
              <span style={{ fontFamily: 'Quicksand, sans-serif', fontWeight: 700, fontSize: 13.5, color: '#123644' }}>
                {t.requests?.address || (t.isSeller ? 'Client' : 'Prestataire')}
              </span>
